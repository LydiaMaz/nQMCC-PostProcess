...
#-----------------------------------------------------------------------
PAIR_NAMES = ["NN (S=0)", "NN (S=1)", "NP (S=0)", "NP (S=1)", "PP (S=0)", "PP (S=1)"]
#-----------------------------------------------------------------------
LATEX_LABELS = {
    "NN (S=0)": r"$\mathrm{nn\ (S=0)}$",
    "NN (S=1)": r"$\mathrm{nn\ (S=1)}$",
    "NP (S=0)": r"$\mathrm{np\ (S=0)}$",
    "NP (S=1)": r"$\mathrm{np\ (S=1)}$",
    "PP (S=0)": r"$\mathrm{pp\ (S=0)}$",
    "PP (S=1)": r"$\mathrm{pp\ (S=1)}$",
}
#-----------------------------------------------------------------------
...
#-----------------------------------------------------------------------
def get_pair_count(filepath):
    """
    Parses the output file from the bottom up to find the "TOTAL AVERAGE" line. 
    Then it parses through the block following the line to identify the "PAIR COUNTS" section, particularly:
     NN (S=0)                NN (S=1)               NN PAIRS
    7.0915 (0.7684E-02)    13.9085 (0.1584E-01)    21.0000 (0.1760E-01)... (or similar)
    then looks at the data line and extracts the values. 
    """
    results = {key: "NaN" for key in PAIR_NAMES}
    ...
    for i in range(tot_avg, len(lines)):
        line = lines[i]
        #matches[0] is nn s=0, matches[1] is nn s=1 (or np, pp)
        for pair in [("NN (S=0)", "NN (S=1)"), ("NP (S=0)", "NP (S=1)"), ("PP (S=0)", "PP (S=1)")]:
...
def build_final_table(grouped_data):
    ...
    first_isotope_entries = list(grouped_data.values())[0]
    potentials = [pot for pot, _ in first_isotope_entries]

    col_alignment = "l" + "c" * len(potentials)
    ...
    for isotope, entries in grouped_data.items():
        out.extend([r"\hline", rf"\multicolumn{{4}}{{c}}{{\textbf{{{isotope}}}}} \\[2pt]", r"\hline"])
        
        for pair_name in PAIR_NAMES:
            row_cells = [LATEX_LABELS[pair_name]]
            
            for potential, data in entries:
                row_cells.append(data.get(pair_name, "NaN"))
